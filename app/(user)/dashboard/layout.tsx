...
import { Package, Home, Box, Truck, ShoppingBag, Store, Tag, HelpCircle, Calculator, BookOpen, User, ChevronRight, LogOut, Menu, X, ShoppingCart, Plane } from 'lucide-react';
...
export default function DashboardLayout({
  ...
  const navigation = [
    ...
    { name: 'Assisted Purchase', href: '/dashboard/assisted-purchase', icon: ShoppingBag },
    { name: 'International Shipment', href: '/dashboard/international-shipment', icon: Plane },
      ...
      `}>
        ...
        <nav className="p-4">
          ...
          <div className="mt-8 pt-8 border-t border-gray-700">
            <div className="text-xs font-semibold text-gray-400 mb-3 px-4">HELP</div>
            <div className="space-y-1">
              {helpLinks.map((item) => {
                const Icon = item.icon;
                return (
          ...
          {/* User Stuff */}
          <div className="mt-8 pt-8 border-t border-gray-700">
            <div className="text-xs font-semibold text-gray-400 mb-3 px-4">YOUR STUFF</div>
            <Link
        ...
        <header className="bg-white border-b px-4 lg:px-8 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-600">
            ...
            <span className="font-semibold text-gray-900 hidden sm:block">
              {pathname === '/dashboard' ? 'Home' : pathname.split('/').pop()?.replace('-', ' ').toUpperCase()}
            </span>
          </div>

          {/* Back Link */}
          <Link href="/" className="text-blue-600 hover:underline text-xs sm:text-sm font-semibold">
